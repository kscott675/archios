...
class Employee < ApplicationRecord
  ...
  def current_timesheet
    today = Date.today
    pay_period = PayPeriod.find_by("start_date <= ? AND end_date >= ?", today, today)

    return {} if pay_period.nil?

    timesheet_entries.joins(:pay_period)
                 .select('timesheet_entries.*, pay_periods.start_date AS pay_period_start_date, pay_periods.end_date AS pay_period_end_date')
